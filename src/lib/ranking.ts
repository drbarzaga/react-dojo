...
export function calculateScore(
  ...
  const conceptPct = Math.min(concepts / totals.concepts, 1)
  const exercisePct = Math.min(exercises / totals.exercises, 1)
  const quizPct = Math.min(quizzes / totals.quizzes, 1)
...
export function getRank(
  ...
  const score = calculateScore(concepts, exercises, quizzes, totals)
